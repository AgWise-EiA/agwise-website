=== SKT Admire ===

Contributors: sonalsinha21
Requires at least: 5.6
Requires PHP:  7.4
Tested up to: 6.4
Version: 1.3
License: GPL-2.0-or-later
License URI: https://www.gnu.org/licenses/license-list.html#GPLv2
Tags:two-columns,right-sidebar,full-width-template,custom-background,custom-colors,custom-menu,sticky-post,theme-options,threaded-comments,featured-images,footer-widgets,translation-ready,rtl-language-support,custom-logo,editor-style,custom-header,blog,e-commerce,portfolio

== Description ==

Admire theme is lightweight fast and easily customizable WordPress theme with compatibility with WooCommerce for eCommerce. Comes prebuilt using Elementor and supports gutenberg block editor as well. Built with SEO in mind and is compatible with SEO plugins. Responsive and Translation Ready. It can easily be extended in terms of functionality with various compatible plugins like contact forms. This is a base theme and has several demos around 50+ in the paid version. It is suitable for building any kind of website for any industry.

== Theme Resources == 

Theme is built using the following resource bundles:

* All js that have been used are within folder /js of theme.

= 	skt-admire/assets/fonts/font-awesome.css
	Font Awesome, Copyright Adobe Dave Gandy =
	Licenses: http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
	Source: http://fontawesome.io

*   Admire Theme Self Designed Images:
	skt-admire/images/admire-logo.png

*   Details of images used in Screenshot:
	https://pxhere.com/en/photo/1639058
	https://pxhere.com/en/photo/670370
	https://pxhere.com/en/photo/1637325
	https://pxhere.com/en/photo/1588417
	https://pxhere.com/en/photo/1603680	
	
= 	Declaring these self designed images under GPL license version 2.0 
=   License URL: http://www.gnu.org/licenses/gpl-2.0.html

*   skt-admire/pro-customize	

		Pro Customize code based on Justintadlock’s Customizer-Pro 
		Source Code URL : https://github.com/justintadlock/trt-customizer-pro			
		License : http://www.gnu.org/licenses/gpl-2.0.html
		Copyright 2016, Justin Tadlock	justintadlock.com
 
For any help you can mail us at support[at]sktthemes.com

== Changelog ==
= 1.3 =
*   Enabled transparent header option on frontpage.

= 1.2 =
*   Resolved all issue as reviewer pointed.

= 1.1 =
*   Solved all the issues that the reviewer pointed out.

= 1.0 =
*   Initial version release. 